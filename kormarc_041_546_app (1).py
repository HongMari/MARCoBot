...
class GptBatchResult:
    """통합 GPT 호출의 결과를 담는 데이터 클래스"""
    lang_a: str = "und"
    lang_h: str = "und"
    keywords_653: list[str] = field(default_factory=list)


def _get_forbidden_list(title: str, author: str) -> list[str]:
    """653 생성을 위해 제목, 저자에서 제외할 단어 목록 생성 (간소화된 버전)"""
    # 제목/저자명을 구두점/슬래시 등으로 분리하여 제외어 목록 생성
    forbidden = [t.strip() for t in re.split(r'[:/?()]+', title) if t.strip()]
    forbidden.extend([a.strip() for a in re.split(r'[:/?()]+', author) if a.strip()])
    return list(set(f for f in forbidden if len(f) >= 2))


def gpt_batch_metadata_analysis(
    title: str, 
    original_title: str, 
    category_text: str, 
    publisher: str, 
    ...
    """
    041 $a, $h 및 653 주제어 생성을 단일 GPT 호출로 통합.
    """
    if not OPENAI_KEY:
        dbg_err("OPENAI_API_KEY가 설정되지 않아 GPT 호출을 건너뜁니다.")
        return GptBatchResult()
        
    # 653 생성을 위한 제외어 목록
    forbidden_list = _get_forbidden_list(title, author)
    
    # $a에 대한 강한 규칙(국내도서=kor, 제목 한글=kor)을 GPT에게 명시적으로 주입
    is_korean_title = detect_language_by_unicode(title) == 'kor'
    domestic_guard = "kor" if is_domestic_category(category_text) or is_korean_title else "und"
    
    prompt = f"""
    아래 도서 정보를 분석하여 필요한 MARC 메타데이터를 JSON 형식으로 추정합니다.
    
    [도서 정보]
    - 제목: {title}
    - 원제: {original_title or "(없음)"}
    - 저자: {author}
    - 분류(전체 체인): {category_text}
    - 출판사: {publisher}
    - 설명: {description[:1000] if description else "(없음)"}
    - 목차: {toc[:1000] if toc else "(없음)"}
    - 제외어 목록(653 주제어 생성 시 제목/저자 유래 등 비주제 요소): {forbidden_list}
    
    [분석 및 지침]
    1. 본문 언어(lang_a): 이 자료의 **현시본(Manifestation)** 언어(kor, eng, jpn 등 ISDS 코드)를 추정.
       * **최우선 강제 규칙**: 자료가 '국내도서'이거나 제목에 한글이 포함된 경우 **무조건 kor**로 추정해야 합니다. 그 외의 경우에만 GPT가 판단합니다. (현재 강제 후보: '{domestic_guard}')
       * 원작 언어(lang_h), 저자 국적 등 **원작 관련 단서 사용 금지**.
    2. 원서 언어(lang_h): 원작 언어를 ISDS 코드로 추정 (번역서가 아니면 lang_a와 동일할 수 있음).
       * 저자 국적·주 집필 언어·최초 출간 언어를 우선 고려.
    3. 653 주제어(keywords_653): 분류/설명/목차에서 핵심 명사 주제어 **최대 5개**를 띄어쓰기 없이 생성.
       * **제외어 목록**({forbidden_list})에 있는 단어나, 시리즈·출판사·판차·연도 등 **비주제 요소는 절대 포함하지 마세요**.
       * 너무 일반적/추상적 표현은 제외하고 구체적인 주제를 선택합니다.
    4. 가능한 ISDS 코드: kor, eng, jpn, chi, rus, fre, ger, ita, spa, por, tur.
    
    [출력 형식 (JSON)] - **반드시 이 형식만 출력**
    {{
      "lang_a": "[ISDS 코드]",
      "lang_h": "[ISDS 코드]",
      "keywords_653": ["키워드1", "키워드2", "키워드3", "키워드4", "키워드5"]
    }}
    """.strip()
    ...
    try:
        resp = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system","content": "MARC 메타데이터(041 $a, $h, 653 주제어)를 통합 분석하여 JSON 형식으로만 반환하는 전문가."},
        ...
        try:
            ...
            # --- $a/$h 코드 정리 ---
            lang_a = data.get("lang_a", "und").lower()
            lang_h = data.get("lang_h", "und").lower()
            
            # $a에 대한 국내도서/한글 제목 강제 규칙 재적용 (GPT가 혹시 틀릴 경우를 대비)
            if domestic_guard == "kor":
                 lang_a = "kor"

            if lang_a not in ALLOWED_CODES: lang_a = "und"
            if lang_h not in ALLOWED_CODES: lang_h = "und"
            
            # --- 653 키워드 정리 ---
            keywords = data.get("keywords_653", [])
            if not isinstance(keywords, list): keywords = []
            keywords = [k.strip().replace(" ", "") for k in keywords if k and len(k.strip()) > 1][:5]
            
            dbg(f"🧭 [GPT 통합 결과] $a={lang_a}, $h={lang_h}, 653={keywords}")
            return GptBatchResult(lang_a=lang_a, lang_h=lang_h, keywords_653=keywords)

        except json.JSONDecodeError as e:
            dbg_err(f"GPT 응답 JSON 파싱 오류: {e}, 원본 응답: {content[:100]}")
            return GptBatchResult()
        
    except Exception as e:
        dbg_err(f"GPT 통합 호출 오류: {e}")
        return GptBatchResult()
...
# ===== 546 생성 (041만 보고) - (get_kormarc_tags에서 호출됨) =====
def generate_546_from_041_kormarc(tag_041: str):
    """041 태그에서 $a와 $h 코드를 추출하여 546 태그 문자열 생성"""
    m = re.search(r"\$a(\w{3})\s+\$h(\w{3})", tag_041)
    if m:
        lang_a_code = m.group(1)
        lang_h_code = m.group(2)
        lang_a_name = ISDS_LANGUAGE_CODES.get(lang_a_code, lang_a_code)
        lang_h_name = ISDS_LANGUAGE_CODES.get(lang_h_code, lang_h_code)
        return f"546 $a{lang_h_name} 원작을 {lang_a_name}로 번역."
    return None

# ===== KORMARC 태그 생성기 (통합 GPT 호출 사용) =====
def get_kormarc_tags(isbn):
    ...
    try:
        ...
        # 653 생성을 위해 필요한 Description 및 TOC (알라딘 API JSON/XML에 없는 경우 있음)
        description = item.findtext("description", default="") or ""
        toc = subinfo.findtext("toc", default="") if subinfo is not None else ""
        
        crawl = crawl_aladin_fallback(isbn)
        if not original_title:
            original_title = crawl.get("original_title", "")
        # subject_lang = crawl.get("subject_lang") # 카테고리 기반 언어 힌트 (이제 GPT 프롬프트에서 직접 활용)
        category_text = crawl.get("category_text", "")
        
        # --- 1. GPT 통합 분석 호출 (041 $a, $h 및 653 주제어) ---
        dbg("📘 [설명] GPT 통합 분석 호출 시작 (041 $a, $h 및 653 주제어)")
        gpt_result = gpt_batch_metadata_analysis(
            title=title,
        ...
        lang_a = gpt_result.lang_a
        lang_h = gpt_result.lang_h
        keywords_653 = gpt_result.keywords_653
        dbg("📘 [결과] 통합 GPT 분석 완료")

        # --- 2. 041 태그 조합 ---
        
        # $a가 kor인데 $h가 다른 언어일 경우 (번역서)
        if lang_h and lang_h != lang_a and lang_h != "und":
            tag_041 = f"041 $a{lang_a} $h{lang_h}"
        else:
            # $a와 $h가 같거나 $h가 und이면 $h는 추가하지 않음 (비번역서)
            tag_041 = f"041 $a{lang_a}"

        # ✅ 번역서($h) 아님 → 041/546 둘 다 만들지 않음
        if "$h" not in tag_041:
            # tag_041, tag_546, original_title, keywords_653
            return tag_041, None, original_title, keywords_653

        # ✅ 번역서일 때만 546 생성
        tag_546 = generate_546_from_041_kormarc(tag_041)

        # 041 태그, 546 태그, 원제, 653 키워드 목록을 반환
        return tag_041, tag_546, original_title, keywords_653
        
    except Exception as e:
        dbg_err(f"📕 예외 발생: {e}")
        # 예외 발생 시 빈 리스트를 포함하여 반환 형식을 유지
        return f"📕 예외 발생: {e}", None, "", []
...
def build_90010_prefer_lod_then_wikidata_with_meta(people: dict, include_translator: bool = True):
    ...
    for nm in names_all:
        ...
        if (not qid) and looks_romanized_korean_name(nm):
            trace.append({"who": nm, "resolved": val, "role": role,
